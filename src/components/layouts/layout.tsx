...
export default function Layout(props: PropsWithChildren) {
  ...
  const navigation = [
    ...
    {
      name: "Wallets",
      pathname: `/project/[projectId]/wallets`,
      icon: Wallet,
    },
    {
      name: "Neurons",
      pathname: `/project/[projectId]/neurons`,
      icon: Brain,
    },
    {
      name: "Traces",
  ...
  if (hideNavigation)
    return (
      <main className="h-full bg-background px-4 py-4 sm:px-6 lg:px-8">
        {props.children}
      </main>
    );
    ...
    <>
      <div>
                            ...
                            {navigation.map((item) => (
                              <li key={item.name}>
                                <Link
                                  ...
                                  className={clsx(
                                    item.current
                                      ? "bg-background-50 text-gray-50"
                                      : "text-gray-400 hover:bg-background-50 hover:text-gray-50",
                              ...
                              <li key={project.name}>
                                ...
                                >
                                  <span
                                    className={cn(
                                      projectId === project.id
                                        ? "border-indigo-600 text-gray-50"
                                        : "border-gray-500 text-gray-400 group-hover:border-indigo-600 group-hover:text-gray-50",
                                      "flex h-6 w-6 shrink-0 items-center justify-center rounded-lg border bg-background p-1 text-[0.625rem] font-medium"
                                    )}
                                  >
                                    <Code />
                                  </span>
        ...
        <div className="hidden lg:fixed lg:inset-y-0 lg:z-50 lg:flex lg:w-72 lg:flex-col">
          {/* Sidebar component, swap this element with another sidebar if you like */}
          <div className="flex grow flex-col gap-y-5 overflow-y-auto border-r border-gray-500 bg-background px-6">
            <div className="flex h-16 shrink-0 items-center font-mono text-xl font-bold">
              🦇 rao.town
            </div>
            <nav className="flex flex-1 flex-col">
              <ul role="list" className="flex flex-1 flex-col gap-y-4">
                ...
                <li className="-mx-6 ">
                  <Menu as="div" className="relative">
                    <Menu.Button className="flex w-full items-center gap-x-4 p-1.5 px-6 py-3 text-sm font-semibold leading-6 text-gray-50 hover:bg-background-50">
                      ...
                      <span className="flex-shrink truncate text-sm font-semibold leading-6 text-gray-50">
                        {session.data?.user?.name}
                      </span>
                      <ChevronDownIcon
                    ...
                    >
                      <Menu.Items className="absolute -top-full right-0 z-10 mt-2.5 w-32 rounded-md bg-background py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                            ...
                            {({ active }) => (
                              <a
                                ...
                                className={cn(
                                  ...
                                  "block px-3 py-1 text-sm leading-6 text-gray-50"
                                )}
                              >
                                {item.name}
                              </a>
        ...
        <div className="sticky top-0 z-40 flex items-center gap-x-6 bg-background px-4 py-4 shadow-sm sm:px-6 lg:hidden">
          ...
          <div className="flex-1 font-mono text-sm font-bold leading-6 text-gray-50">
            🦇 rao.town
          </div>
          <Menu as="div" className="relative">
            <Menu.Button className="flex items-center gap-x-4 text-sm font-semibold leading-6 text-gray-50">
              <span className="sr-only">Open user menu</span>
              <Avatar className="h-8 w-8">
